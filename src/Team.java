...
public class Team {
  ...
  private void setAvgPoints() {
    ...
    if (Main.activeTripleChip()) {
      ...
      sum += drivers.get(1)
          .getAvgPoints(); // Double highest driver (not accurate but will help simulate 2x chip)
    } else {
      sum += drivers.get(0)
          .getAvgPoints(); // Double highest driver (not accurate but will help simulate 2x chip)
    }
  ...
  private void setAvgRecPoints() {
    ...
    if (Main.activeTripleChip()) {
      sum += 2 * drivers.get(0)
          .getAvgRecPoints(); // Triple highest driver (not accurate but will help simulate 3x chip)
      sum += drivers.get(1)
          .getAvgRecPoints(); // Double second driver (not accurate but will help simulate 2x chip)
    } else {
      sum += drivers.get(0)
          .getAvgRecPoints(); // Double highest driver (not accurate but will help simulate 2x chip)
    }
  ...
  private void setTransferCount() {
    ...
    for (Driver d : drivers) {
      if (!comparableTeam.drivers.contains(d)) {
        transferCount++;
      }
    }
    // Count constructors in this team not in the comparableTeam
    for (Constructor c : constructors) {
      if (!comparableTeam.constructors.contains(c)) {
        transferCount++;
      }
    }
