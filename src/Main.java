...
public class Main {
  ...
  // Chips
  private static final boolean tripleChip = true;
  ...
  private static final int maxTransfers = 3;
  ...
  public static void main(String[] args) {
    ...
    for (Driver d : Driver.values()) {
      System.out.println(d.toInfo());
    }
    for (Constructor c : Constructor.values()) {
      System.out.println(c.toInfo());
    }
    ...
    System.out.println("\nRestrictions:");
    System.out.println(
        " - Cost Cap {" + Utility.formatMoney(comparableTeam.getCurrentCost()) + "} [" + (
            checkForCost ? "ACTIVE" : "NOT ACTIVE") + "]");
    System.out.println(
        " - Max Transfers {" + maxTransfers + "} [" + (checkForTransfers ? "ACTIVE" : "NOT ACTIVE")
            + "]");
    System.out.println("\nAdd-ons:");
    ...
    System.out.print("\n\nAnalysing .");
    allTeams.sort(Comparator.comparing(Team::getCostDifference)
        .reversed()); // Prioritise more expensive teams
    System.out.print(" .");
    allTeams.sort(Comparator.comparing(Team::getTransferCount)); // Prioritise less transfers
    System.out.print(" .");
    allTeams.sort(
        Comparator.comparing(Team::getAvgPoints).reversed()); // Prioritise more avg points
    System.out.print(" .");
    allTeams.sort(
        Comparator.comparing(Team::getAvgRecPoints).reversed()); // Prioritise more avg rec points
    ...
    allTeams.sort(
        Comparator.comparing(Team::getAvgPoints).reversed()); // Prioritise more avg points
    displayOrder(allTeams, 5, 3);

    LocalTime end = LocalTime.now();
    System.out.println(
        "Finished in (" + ((float) ((end.toNanoOfDay() - start.toNanoOfDay()) / 1000000) / 1000)
            + "s)");
  }
  ...
  public static ArrayList<Team> generateAllCombinations(Driver[] drivers,
    ...
    generateDriverCombinations(drivers, selectedDrivers, allTeams, constructors,
        selectedConstructors, 0);
    return allTeams;
  }

  private static void generateDriverCombinations(Driver[] drivers, List<Driver> currentDrivers,
      ArrayList<Team> allTeams, Constructor[] constructors, List<Constructor> currentConstructors,
      int driverStart) {
    ...
    for (int i = driverStart; i < drivers.length; i++) {
      ...
      generateDriverCombinations(drivers, currentDrivers, allTeams, constructors,
          currentConstructors, i + 1);
      currentDrivers.remove(currentDrivers.size() - 1);
    }
  }

  private static void generateConstructorCombinations(Constructor[] constructors,
      List<Constructor> currentConstructors, ArrayList<Team> allTeams, List<Driver> currentDrivers,
      int constructorStart) {
    if (currentConstructors.size() == 2) { // Now each team needs exactly 2 constructors
      Team newTeam = new Team("Team" + teamNumber++, new ArrayList<>(currentDrivers),
          new ArrayList<>(currentConstructors));
      if ((!checkForCost || !newTeam.isTooExpensive()) && (!checkForTransfers
          || newTeam.getTransferCount() <= maxTransfers)) {
